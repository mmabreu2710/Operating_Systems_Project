...
static pthread_cond_t condition_fa;
static int ficheiros_abertos;
static int flag_tfs_destroy_aac;
int tempo;
int tfs_init() {
    state_init();
    flag_tfs_destroy_aac = 0;
    if (pthread_mutex_init(&single_global_lock, 0) != 0)
        return -1;
    if (pthread_cond_init(&condition_fa, 0) != 0)
        return -1;
    /* create root inode */
...
int tfs_destroy() {
    ...
    if (pthread_cond_destroy(&condition_fa) != 0) {
        return -1;
    }
    return 0;
}
...
int tfs_destroy_after_all_closed() {
    ...
    flag_tfs_destroy_aac = 1;
    while (ficheiros_abertos>0){
        pthread_cond_wait(&condition_fa, &single_global_lock);
    }
...
int tfs_close(int fhandle) {
    ...
    if (ficheiros_abertos == 0){
        pthread_cond_broadcast(&condition_fa);
    }
    if (pthread_mutex_unlock(&single_global_lock) != 0)
        return -1;
...
static ssize_t _tfs_write_unsynchronized(int fhandle, void const *buffer,
    ...
    inode->ultima_vez_acedida = tempo;
